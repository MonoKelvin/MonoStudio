  ...
  <div class="base-image-view" :class="{ 'fullscreen': isFullscreen }">
    <div class="image-toolbar" v-if="showToolbar">
      <div class="toolbar-left">
        <BaseButton size="sm" variant="default" @click="handleClose" class="close-btn">
          ✕
        </BaseButton>
        <span class="image-info">{{ imageInfo }}</span>
      </div>
      <div class="toolbar-right">
        <BaseButton size="sm" variant="default" @click="handleZoomOut" :disabled="scale <= 0.25" title="缩小">
          −
        </BaseButton>
        <span class="scale-info">{{ Math.round(scale * 100) }}%</span>
        <BaseButton size="sm" variant="default" @click="handleZoomIn" :disabled="scale >= 3" title="放大">
          +
        </BaseButton>
        <BaseButton size="sm" variant="default" @click="handleReset" title="重置">
          ⟲
        </BaseButton>
        <BaseButton size="sm" variant="default" @click="toggleFullscreen" title="全屏">
          {{ isFullscreen ? '⊠' : '⛶' }}
        </BaseButton>
      </div>
    </div>
    ...
    <div class="image-container" @wheel.prevent="handleWheel">
      <img ref="imageRef" :src="src" :alt="alt" :style="imageStyle" @load="handleImageLoad" @error="handleImageError" />
    ...
    <div class="image-actions" v-if="showActions && !hasError">
      <BaseButton variant="primary" size="sm" @click="handleSave" class="action-btn">
        <span>💾</span>
        另存为
      </BaseButton>
      <BaseButton variant="default" size="sm" @click="handleCopy" class="action-btn">
        <span>📋</span>
        复制
      </BaseButton>
    </div>
