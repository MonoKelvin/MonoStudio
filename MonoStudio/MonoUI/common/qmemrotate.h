...
static inline void qt_memrotate90_tiled(const T* src, int w, int h, int sstride, T* dest, int dstride)
{
    ...
    const int restX = w % sTileSize;
    const int restY = (h - unaligned) % sTileSize;
    const int unoptimizedY = restY % pack;
    const int numTilesX = w / sTileSize + (restX > 0);
    const int numTilesY = (h - unaligned) / sTileSize + (restY >= pack);

    for (int tx = 0; tx < numTilesX; ++tx)
    {
        const int startx = w - tx * sTileSize - 1;
        const int stopx = qMax(startx - sTileSize, 0);
        ...
        {
            const int starty = ty * sTileSize + unaligned;
            const int stopy = qMin(starty + sTileSize, h - unoptimizedY);
...
static inline void qt_memrotate90_tiled_unpacked(const T* src, int w, int h, int sstride, T* dest,
        int dstride)
{
    const int numTilesX = (w + sTileSize - 1) / sTileSize;
    const int numTilesY = (h + sTileSize - 1) / sTileSize;
    ...
    for (int tx = 0; tx < numTilesX; ++tx)
    {
        ...
        for (int ty = 0; ty < numTilesY; ++ty)
        {
            const int starty = ty * sTileSize;
            const int stopy = qMin(starty + sTileSize, h);
...
static inline void qt_memrotate270_tiled(const T* src, int w, int h, int sstride, T* dest, int dstride)
{
    ...
    for (int tx = 0; tx < numTilesX; ++tx)
    {
        const int startx = tx * sTileSize;
        const int stopx = qMin(startx + sTileSize, w);
        ...
        for (int ty = 0; ty < numTilesY; ++ty)
        {
            const int starty = h - 1 - unaligned - ty * sTileSize;
            const int stopy = qMax(starty - sTileSize, unoptimizedY);
        ...
        int dstride)
{
    ...
    for (int tx = 0; tx < numTilesX; ++tx)
    {
        ...
        for (int ty = 0; ty < numTilesY; ++ty)
        {
            const int starty = h - 1 - ty * sTileSize;
            const int stopy = qMax(starty - sTileSize, 0);
...
template <class T>
static inline void qt_memrotate270_template(const T* src, int srcWidth, int srcHeight, int srcStride,
        T* dest, int dstStride)
{
#if Q_BYTE_ORDER == Q_LITTLE_ENDIAN
    // packed algorithm assumes little endian and that sizeof(quint32)/sizeof(T) is an integer
...
inline void qt_memrotate90(const type *src, int w, int h, int sstride, \
                                 type *dest, int dstride)           \
{                                                                   \
    qt_memrotate90_template(src, w, h, sstride, dest, dstride);     \
}                                                                   \
inline void qt_memrotate180(const type *src, int w, int h, int sstride, \
                                  type *dest, int dstride)          \
{                                                                   \
    qt_memrotate180_template(src, w, h, sstride, dest, dstride);    \
}                                                                   \
inline void qt_memrotate270(const type *src, int w, int h, int sstride, \
                                  type *dest, int dstride)          \
{                                                                   \
...
inline void qt_memrotate90_8(const uchar* srcPixels, int w, int h, int sbpl, uchar* destPixels, int dbpl)
{
    qt_memrotate90(srcPixels, w, h, sbpl, destPixels, dbpl);
}

inline void qt_memrotate180_8(const uchar* srcPixels, int w, int h, int sbpl, uchar* destPixels, int dbpl)
{
    qt_memrotate180(srcPixels, w, h, sbpl, destPixels, dbpl);
}

inline void qt_memrotate270_8(const uchar* srcPixels, int w, int h, int sbpl, uchar* destPixels, int dbpl)
{
    qt_memrotate270(srcPixels, w, h, sbpl, destPixels, dbpl);
}

inline void qt_memrotate90_16(const uchar* srcPixels, int w, int h, int sbpl, uchar* destPixels, int dbpl)
{
    qt_memrotate90((const ushort*)srcPixels, w, h, sbpl, (ushort*)destPixels, dbpl);
}

inline void qt_memrotate180_16(const uchar* srcPixels, int w, int h, int sbpl, uchar* destPixels, int dbpl)
{
    qt_memrotate180((const ushort*)srcPixels, w, h, sbpl, (ushort*)destPixels, dbpl);
}

inline void qt_memrotate270_16(const uchar* srcPixels, int w, int h, int sbpl, uchar* destPixels, int dbpl)
{
    qt_memrotate270((const ushort*)srcPixels, w, h, sbpl, (ushort*)destPixels, dbpl);
}

inline void qt_memrotate90_32(const uchar* srcPixels, int w, int h, int sbpl, uchar* destPixels, int dbpl)
{
    qt_memrotate90((const uint*)srcPixels, w, h, sbpl, (uint*)destPixels, dbpl);
}

inline void qt_memrotate180_32(const uchar* srcPixels, int w, int h, int sbpl, uchar* destPixels, int dbpl)
{
    qt_memrotate180((const uint*)srcPixels, w, h, sbpl, (uint*)destPixels, dbpl);
}

inline void qt_memrotate270_32(const uchar* srcPixels, int w, int h, int sbpl, uchar* destPixels, int dbpl)
{
    qt_memrotate270((const uint*)srcPixels, w, h, sbpl, (uint*)destPixels, dbpl);
}


inline void qt_memrotate90_64(const uchar* srcPixels, int w, int h, int sbpl, uchar* destPixels, int dbpl)
{
    qt_memrotate90((const quint64*)srcPixels, w, h, sbpl, (quint64*)destPixels, dbpl);
}

inline void qt_memrotate180_64(const uchar* srcPixels, int w, int h, int sbpl, uchar* destPixels, int dbpl)
{
    qt_memrotate180((const quint64*)srcPixels, w, h, sbpl, (quint64*)destPixels, dbpl);
}

inline void qt_memrotate270_64(const uchar* srcPixels, int w, int h, int sbpl, uchar* destPixels, int dbpl)
{
